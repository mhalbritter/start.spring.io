...
@ConditionalOnRequestedDependency("jte-starter")
class JteProjectGenerationConfiguration {
	...
	BuildCustomizer<Build> jteBuildCustomizer() {
		...
		return (build) -> {
			Dependency jteStarter = build.dependencies().get("jte-starter");
			build.dependencies()
				.add("jte", Dependency.withCoordinates("gg.jte", "jte").version(jteStarter.getVersion()).build());
		};
	}
